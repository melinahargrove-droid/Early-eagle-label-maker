(()=>{
  ...
  function syncStudents(){const names=parseNames();const old=new Map(students.map(s=>[s.name,s]));students=names.map(name=>old.get(name)||{name,photo:''});renderStudents();updateCount();renderPreview();}
