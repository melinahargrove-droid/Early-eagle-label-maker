  ...
  async function forceCurrentWorker(){
    ...
    try{
      ...
      if('caches' in window){
        ...
        await Promise.all(keys.filter(k=>k.startsWith('early-eagle-label-maker-')).map(k=>caches.delete(k)));
      }
      ...
      updateVisibleBuild();
    }catch(err){console.warn('PWA version refresh failed:',err);updateVisibleBuild();}
  }
